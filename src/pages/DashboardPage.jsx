...
export function DashboardPage() {
  ...
  // Permission names below must match what auth_login returns in user_permission_names.
  // AdminPanel.jsx still writes to the legacy `permissions` jsonb with keys like
  // can_view_revenue — migration 05 mirrored existing flags into user_permissions rows,
  // so current users keep access. New grants via AdminPanel won't reach the new table
  // until AdminPanel is rewritten in Subplan 2.
  const canViewRevenue = hasPermission(user, 'view_all_revenue')
  const canViewChart   = hasPermission(user, 'view_chart')
