...
function getItemKey(moduleId, item) {
  return `${moduleId}:${item.label}`;
}

function getDefaultItemKey(module) {
...
function getActiveItemLabel(module, activeItemKey) {
  const items = module.sections.flatMap((section) => section.items);
  const match = items.find((item) => getItemKey(module.id, item) === activeItemKey);
        ...
        <div className="space-y-2" key={section.title ?? index}>
          ...
          <SidebarMenu>
            {section.items.map((item) => {
              ...
              return (
                <SidebarMenuItem key={item.label} className="text-muted-foreground hover:text-foreground">
                  <SidebarMenuButton
                    ...
                    render={<a
                      ...
                      onClick={(e) => {
                                                ...
                                                }} />}><Icon className="size-4" />{item.badge && (
            ...
            return (
              <SidebarMenuItem key={item.label}>
                <SidebarMenuButton
                  ...
                  render={<a href={item.href} />}><Icon className="size-4" /><span>{item.label}</span></SidebarMenuButton>
              </SidebarMenuItem>
            );
          })}
        </SidebarMenu>
  ...
  return (
    <Sidebar
      ...
      <Sidebar
        ...
        <SidebarHeader className="h-14 border-b">
          <SidebarMenu className="my-auto">
            <SidebarMenuItem>
              <SidebarMenuButton size="lg" className="md:h-8 md:p-0" render={<a href="#" />}><div
                className="flex aspect-square size-8 items-center justify-center rounded-sm bg-primary">
                                              <img
                                                src="https://deifkwefumgah.cloudfront.net/shadcnblocks/block/logos/shadcnblocks-logo.svg"
                                                alt="Shadcnblocks"
                                                className="size-6 text-primary-foreground invert dark:invert-0" />
                                            </div><span className="text-lg font-semibold tracking-tighter">
                                              Shadcnblocks
      ...
      <Sidebar collapsible="none" className="hidden flex-1 md:flex">
        <SidebarHeader
          className={cn("flex h-14 border-b px-2", isCollapsed
          ...
          <SidebarMenu>
            <SidebarMenuItem>
              <DropdownMenu>
                <div
                  className="flex w-full items-center px-2 group-data-[collapsible=icon]:justify-center">
                  <DropdownMenuTrigger
                    render={<Button
                      variant="ghost"
                      className="flex h-auto items-center gap-1.5 p-0! hover:bg-transparent" />}><span className="font-semibold">Shadcnblocks</span><ChevronDown className="size-3 text-muted-foreground" /></DropdownMenuTrigger>
                </div>
                <DropdownMenuContent className="min-w-60 rounded-lg" side="bottom" align="center">
                  <DropdownMenuGroup>
                    <DropdownMenuItem render={<a href="#" />}>Workspace settings
                                                              <DropdownMenuShortcut>⌥⇧S</DropdownMenuShortcut></DropdownMenuItem>
                    <DropdownMenuSub>
                      <DropdownMenuSubTrigger>
                        Invite teammates
                      </DropdownMenuSubTrigger>
                      <DropdownMenuPortal>
                        <DropdownMenuSubContent className="min-w-64 rounded-lg">
                          ...
                          <DropdownMenuItem className="flex items-center gap-2">
                            <div
                              className="flex aspect-square size-6 items-center justify-center rounded bg-blue-500 text-[10px] font-semibold text-white">
                              EM
                            </div>
                            Invite by email
                          </DropdownMenuItem>
                          <DropdownMenuItem className="flex items-center gap-2">
                            <div
                              className="flex aspect-square size-6 items-center justify-center rounded bg-emerald-500 text-[10px] font-semibold text-white">
                              LK
                            </div>
                            Copy invite link
        ...
        <DropdownMenu>
          <DropdownMenuTrigger
            render={<SidebarMenuButton
              size="default"
              className="h-10 w-full data-[state=open]:bg-sidebar-accent data-[state=open]:text-sidebar-accent-foreground" />}><Avatar className="h-7 w-7">
                                  <AvatarImage src={user.avatar} alt={user.name} />
                                  <AvatarFallback
                                    className="bg-sidebar-primary text-xs font-medium text-sidebar-primary-foreground">
                                    {initials}
                                  </AvatarFallback>
                                </Avatar><span className="min-w-0 flex-1 truncate text-sm font-medium">
                                  {user.name}
                                </span><ChevronDown className="ml-auto size-4 text-muted-foreground" /></DropdownMenuTrigger>
          <DropdownMenuContent
            ...
            <DropdownMenuLabel className="p-0 font-normal">
              <div className="flex items-center gap-2 px-1 py-1.5 text-left text-sm">
                <Avatar className="h-8 w-8">
                  <AvatarImage src={user.avatar} alt={user.name} />
                  <AvatarFallback
                    className="bg-sidebar-primary font-medium text-sidebar-primary-foreground">
                    {initials}
                  </AvatarFallback>
                </Avatar>
          ...
          {data.participants.map((participant) => {
            ...
            return (
              <button
                ...
                <span
                  className={cn("mt-0.5 size-2 shrink-0 rounded-full", statusClassName)}
                  aria-hidden="true" />
      ...
      <section className="space-y-3">
        ...
        <div className="space-y-2">
          {data.activity.map((event) => {
            ...
            return (
              <div key={event.id} className="flex gap-3 rounded-md p-2 hover:bg-accent/50">
                <div
                  className="mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-md bg-accent/50">
...
export function ApplicationShell7({
  className
}) {
  const [activeKey, setActiveKey] = React.useState(navModules[0].id);
  ...
  return (
    <SidebarProvider
      ...
      <SidebarInset>
        <header
          className="sticky top-0 flex h-14 shrink-0 items-center gap-2 border-b bg-background px-3">
          ...
          <div className="flex flex-1 items-center gap-2">
            <ShellBreadcrumb moduleLabel={activeModule.label} itemLabel={activeItemLabel} />
          </div>
          <Sheet>
            <SheetTrigger render={<Button variant="ghost" size="icon" className="lg:hidden" />}><Users className="size-5" /></SheetTrigger>
      ...
      <SidebarRight />
      <Drawer open={isMobilePanelOpen} onOpenChange={setIsMobilePanelOpen} dismissible>
