-- Migration 104: extend list_user_notifications + count_user_notifications_unseen
-- to include staff_activity as a 4th source.
--
-- Per-role scoping:
--   superadmin → all events
--   admin      → events on staff in admin's assigned agencies
--   other      → only events about themselves (sa.user_id = caller)
  ...
  WITH all_events AS (
    ...
    WHERE tma.actor_id IS DISTINCT FROM v_caller_id
      AND CASE
            ...
            ELSE
              tm.lead_user_id = v_caller_id
              OR EXISTS (SELECT 1 FROM team_members mem WHERE mem.team_id = tm.id AND mem.operator_id = v_caller_id)
          END

    UNION ALL

    SELECT
      ('staff_activity:' || sa.id)::text,
      'staff_activity'::text,
      sa.user_id,
      (target_u.first_name || ' ' || COALESCE(target_u.last_name, ''))::text,
      sa.actor_id,
      (actor_u.first_name || ' ' || COALESCE(actor_u.last_name, ''))::text,
      sa.event_type, sa.payload, sa.created_at
    FROM staff_activity sa
    JOIN dashboard_users target_u    ON target_u.id = sa.user_id
    LEFT JOIN dashboard_users actor_u ON actor_u.id = sa.actor_id
    WHERE sa.actor_id IS DISTINCT FROM v_caller_id
      AND CASE
            WHEN v_role = 'superadmin' THEN true
            WHEN v_role = 'admin' THEN
              target_u.agency_id IS NOT NULL
              AND target_u.agency_id IN (SELECT agency_id FROM admin_agencies WHERE admin_id = v_caller_id)
            ELSE
              sa.user_id = v_caller_id
          END
    ...
    SELECT 1 FROM team_activity tma
     ...
     WHERE tma.created_at > v_last_seen
       ...
       AND CASE
             ...
             WHEN v_role = 'admin' THEN
               tm.agency_id IN (SELECT agency_id FROM admin_agencies WHERE admin_id = v_caller_id)
             ELSE tm.lead_user_id = v_caller_id
                  OR EXISTS (SELECT 1 FROM team_members mem WHERE mem.team_id = tm.id AND mem.operator_id = v_caller_id)
           END
    UNION ALL
    SELECT 1 FROM staff_activity sa
      JOIN dashboard_users target_u ON target_u.id = sa.user_id
     WHERE sa.created_at > v_last_seen
       AND sa.actor_id IS DISTINCT FROM v_caller_id
       AND CASE
             WHEN v_role = 'superadmin' THEN true
             WHEN v_role = 'admin' THEN
               target_u.agency_id IS NOT NULL
               AND target_u.agency_id IN (SELECT agency_id FROM admin_agencies WHERE admin_id = v_caller_id)
             ELSE sa.user_id = v_caller_id
           END
    UNION ALL
