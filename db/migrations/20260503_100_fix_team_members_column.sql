-- Migration 100: fix team_members column reference (operator_id, not user_id).
  ...
  WITH all_events AS (
    ...
    WHERE tma.actor_id IS DISTINCT FROM v_caller_id
      AND CASE
            ...
            ELSE
              EXISTS (SELECT 1 FROM team_members mem WHERE mem.team_id = tm.id AND mem.operator_id = v_caller_id)
          END
    ...
    SELECT 1 FROM team_activity tma
     ...
     WHERE tma.created_at > v_last_seen
       ...
       AND CASE
             ...
             WHEN v_role = 'admin' THEN
               tm.agency_id IN (SELECT agency_id FROM admin_agencies WHERE admin_id = v_caller_id)
             ELSE EXISTS (SELECT 1 FROM team_members mem WHERE mem.team_id = tm.id AND mem.operator_id = v_caller_id)
           END
    UNION ALL
